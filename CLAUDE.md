...
### Callout Management
The repository includes several Python utilities for managing callout content:

**Extract callouts to CSV:**
```bash
python callouts.py . callouts.csv
```
Extracts all Quarto callout blocks from `.qmd` files and exports them to CSV for editorial review.

**Update callouts from master CSV (authoritative source):**
```bash
python3 update-callouts-from-csv.py
```
Uses `callouts-master.csv` as the source of truth — removes existing callouts from trail files and reinserts them with proper Quarto syntax and media comments. `callouts-master.csv` is the canonical database for callout content.

**Insert illustration image markers into callouts:**
```bash
python3 insert-illustrations.py
```
Matches callout titles to illustration filenames and inserts `<!-- IMAGE: ... -->` comments.

**Export chapters without callouts (for DOCX delivery):**
```bash
./create-chapters-no-callouts.sh
```
Creates DOCX exports with callout blocks stripped out, stored in `trail_backups_nocallouts/`.

**Remove images from chapters:**
```bash
python3 remove-images-from-chapters.py
```
...
- **`publisher_images/`**: Clean image set for publisher delivery (generated by `collect-images.py`)
...
- **Trail file numbering**: The trail filenames on disk (e.g., `Trail_13__Aspire_Park.qmd`) may differ from what `_quarto.yml` references (e.g., `Trail_13__Emory_Falls.qmd`) due to past renumbering. Always check `_quarto.yml` as the authoritative chapter order. See `TRAIL_RENUMBERING_SUMMARY.txt` for history.
...
- GitHub Actions workflow (`publish.yml`) auto-renders and deploys on push to `master` branch (not `main`)
- `.Rproj` file is a legacy artifact; current workflow is pure Quarto
